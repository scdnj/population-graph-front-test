...
const {
  ...
  checkedPrefectures: getCheckedPrefectures,
} = useComposition();
    ...
    <Section title="都道府県">
      <div class="flex flex-wrap text-lg font-bold">
        <div v-for="prefecture in prefectures" :key="prefecture.prefCode" class="w-1/3 sm:w-24">
          <Checkbox
            :model-value="getCheckedPrefectures[prefecture.prefCode]"
            ...
            @update:model-value="setCheckedPrefectures(prefecture.prefCode, $event)"
          />
        </div>
      </div>
    </Section>
